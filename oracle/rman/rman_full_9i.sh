...
do
	...
	grep ERROR /backup/logs/$ORACLE_SID/$ORACLE_SID.FULL.$DATE.log
		if [ "$?" -eq "0" ]
		then
			cat /backup/logs/$ORACLE_SID/$ORACLE_SID.FULL.$DATE.log | mail -s "FULL BACKUP ERROR - $HOSTNAME - DB: $ORACLE_SID" alessandro.greganti@unito.it
		else
			cat /backup/logs/$ORACLE_SID/$ORACLE_SID.FULL.$DATE.log | mail -s "FULL BACKUP SUCCEDED - $HOSTNAME - DB: $ORACLE_SID" alessandro.greganti@unito.it
		fi
done
